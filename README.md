...
把 **LINE 聊天記錄裡的見證/分享**，經 Claude 產草稿、人工確認後**發佈成文章**，提供：

- **公開網站**：列出全部聖靈故事、文章詳情頁。
- **公開 API**：給手機桌面 widget 取「最新一篇」。
- **後台**：匯入 LINE、搜尋挑訊息、產草稿、編修、發佈。

> 內容管線：LINE 記錄 →（後台篩選 + Claude 產草稿）→ 你確認發佈 → 網站 + Widget 顯示最新一篇。
...
```
backend/   FastAPI + SQLite(FTS5 trigram) + Anthropic SDK
  parser.py / importer.py   解析＆匯入 LINE 匯出檔（去重）
  db.py                     messages + FTS、articles CRUD/發佈
  summarizer.py             Claude 把對話改寫成文章草稿
...
cp .env.example .env     # 填 ANTHROPIC_API_KEY、ADMIN_PASSWORD、SECRET_KEY、PUBLIC_BASE_URL
...
2. `/admin/ingest`：上傳 LINE 匯出 `.txt` → 搜尋、勾選要用的訊息 → 「產生文章草稿」（送 Claude）。
...
docker run -p 8000:8000 -v hs_data:/data \
  -e ADMIN_PASSWORD=... -e SECRET_KEY=... -e ANTHROPIC_API_KEY=... \
...
## 測試
```bash
.venv/bin/python -m pytest -q      # parser / articles / auth / API（draft 以 mock 取代 Claude）
```

## 隱私與安全
- 資料存本機/自架 SQLite；只有後台選取要產草稿的訊息會送到 Anthropic API。
